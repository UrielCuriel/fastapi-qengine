...
from fastapi_qengine.core.types import FilterFormat, ComparisonOperator, LogicalOperator
...
class TestASTBuilder:
    ...
    def test_build_simple_condition(self):
        ...
        assert hasattr(ast.where, 'field')
        assert ast.where.field == "category"
        assert ast.where.operator == ComparisonOperator.EQ
        assert ast.where.value == "electronics"
    ...
    def test_build_logical_condition(self, sample_filter_data):
        ...
        assert ast.where.operator == LogicalOperator.OR
    ...
    def test_build_invalid_operator(self):
        ...
        from fastapi_qengine.core.types import FilterInput, FilterFormat
        invalid_input = FilterInput(
            where={"price": {"$invalid": 50}},
