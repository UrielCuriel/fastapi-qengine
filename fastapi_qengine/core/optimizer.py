...
from .types import (
    FilterAST, ASTNode, FieldCondition, LogicalCondition, OrderNode,
...
class ASTOptimizer:
    ...
    def _optimize_logical_condition(self, node: LogicalCondition) -> ASTNode:
    ...
    def _simplify_logical_operators(self, operator: LogicalOperator, 
                                   conditions: List[ASTNode]) -> List[ASTNode]:
        """Simplify nested logical operators of the same type."""
        simplified = []
        
        for condition in conditions:
            if (isinstance(condition, LogicalCondition) and 
    ...
    def _nodes_equal(self, node1: Optional[ASTNode], node2: Optional[ASTNode]) -> bool:
        ...
        if isinstance(node1, FieldCondition) and isinstance(node2, FieldCondition):
            return (node1.field == node2.field and 
                   node1.operator == node2.operator and 
            ...
            if node1.operator in [LogicalOperator.AND, LogicalOperator.OR]:
                # For commutative operators, check if sets are equal
                keys1 = [self._get_condition_key(c) for c in node1.conditions]
                keys2 = [self._get_condition_key(c) for c in node2.conditions]
                return sorted(keys1) == sorted(keys2)
            else:
                # For non-commutative operators, check order
                return all(self._nodes_equal(c1, c2) for c1, c2 in 
                          zip(node1.conditions, node2.conditions))
