...
from .types import FilterInput, FilterDict, OrderSpec, FieldsSpec
        ...
        for key, value in condition.items():
            if key in ['$and', '$or']:
                if isinstance(value, list):
                    ...
                    # Remove single-item logical operators
                    if len(simplified_items) == 1:
                        # Merge single $and/$or into parent
                        single_item = simplified_items[0]
                        if isinstance(single_item, dict):
                            simplified.update(single_item)
                        else:
                            simplified[key] = simplified_items
                    else:
                        simplified[key] = simplified_items
                else:
                ...
                if isinstance(value, dict):
                    simplified[key] = self._simplify_logical_operators(value)
                else:
                    simplified[key] = value
