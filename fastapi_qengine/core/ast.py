...
from .types import (
    FilterInput, FilterAST, ASTNode, FieldCondition, LogicalCondition,
    OrderNode, FieldsNode, ComparisonOperator, LogicalOperator
)
...
class ASTBuilder:
    ...
    def _build_condition_node(self, condition: Dict[str, Any]) -> ASTNode:
        ...
        for key, value in condition.items():
            if key.startswith('$') and key in ['$and', '$or', '$nor']:
                ...
                logical_operators.append(LogicalCondition(
                    operator=logical_op,
    ...
    def _build_field_condition(self, field: str, condition: Any) -> FieldCondition:
        ...
        if isinstance(condition, dict):
            ...
            if len(condition) == 1:
                ...
                try:
                    operator = ComparisonOperator(op_key)
                except ValueError:
                ...
                # Multiple operators - need to split into multiple conditions
                # For now, we'll combine them with AND
                conditions = []
