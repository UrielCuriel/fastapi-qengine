...
def register_custom_operator(name: str, handler: CustomOperatorHandler) -> None:
    ...
    operator_registry.register_operator(
        ...
        backends=handler.supported_backends
    )


def create_simple_operator(name: str, mongo_impl: Callable[[str, Any], Dict], 
...
def register_builtin_custom_operators():
    ...
    create_simple_operator(
        '$text',
        lambda field, value: {'$text': {'$search': value}},
        ['beanie', 'pymongo']
    )
    ...
    create_simple_operator(
        '$geoWithin',
        lambda field, value: {field: {'$geoWithin': value}},
        ['beanie', 'pymongo']
    )
    ...
    create_simple_operator(
        '$near',
        lambda field, value: {field: {'$near': value}},
        ['beanie', 'pymongo']
    )
...
def get_operator_handler(operator_name: str, backend: str = None) -> Any:
